...
func TestReCompleteAfterUncomplete(t *testing.T) {
	...
	service := NewTaskService(NewPostgresTaskRepository(pool), nil, nil)
...
// date with an already-active completion keeps failing with ErrCompletionAlreadyExists.
func TestDoubleCompleteStillConflicts(t *testing.T) {
	...
	if _, err := service.CompleteTask(ctx, habit.ID, date, nil); err == nil {
		t.Fatal("second CompleteTask: want ErrCompletionAlreadyExists, got nil")
	} else if !errors.Is(err, ErrCompletionAlreadyExists) {
		t.Fatalf("second CompleteTask: want ErrCompletionAlreadyExists, got %v", err)
	}
}
