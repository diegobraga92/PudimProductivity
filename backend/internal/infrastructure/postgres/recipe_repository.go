...
func scanRecipe(scanner interface{ Scan(dest ...any) error }) (*recipedomain.Recipe, error) {
...
func (r *RecipeRepository) Create(ctx context.Context, recipe *recipedomain.Recipe) error {
...
func insertChildren(ctx context.Context, tx pgx.Tx, recipe *recipedomain.Recipe) error {
...
func (r *RecipeRepository) GetByID(ctx context.Context, id string) (*recipedomain.Recipe, error) {
	row := r.pool.QueryRow(ctx, `
		SELECT `+recipeColumns+` FROM recipes WHERE id = $1`, id)
	recipe, err := scanRecipe(row)
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, recipedomain.ErrNotFound
	}
	if err != nil {
...
func (r *RecipeRepository) loadTags(ctx context.Context, recipeID string) ([]string, error) {
...
func (r *RecipeRepository) loadIngredients(ctx context.Context, recipeID string) ([]recipedomain.Ingredient, error) {
	rows, err := r.pool.Query(ctx, `
	...
	var out []recipedomain.Ingredient
	for rows.Next() {
		var ing recipedomain.Ingredient
...
func (r *RecipeRepository) loadSteps(ctx context.Context, recipeID string) ([]recipedomain.Step, error) {
	rows, err := r.pool.Query(ctx, `
	...
	var out []recipedomain.Step
	for rows.Next() {
		var s recipedomain.Step
...
func (r *RecipeRepository) List(ctx context.Context, filter recipedomain.ListFilter) ([]*recipedomain.Recipe, error) {
	limit := filter.Limit
...
func (r *RecipeRepository) Update(ctx context.Context, recipe *recipedomain.Recipe) error {
	tx, err := r.pool.Begin(ctx)
	if err != nil {
		return fmt.Errorf("begin tx: %w", err)
	}
