...
func (s *LibraryService) Create(ctx context.Context, in CreateInput) (*Item, error) {
	item, err := NewItem(uuid.NewUUID(), in.Name, in.MediaType, in.ReleaseYear, in.Done, in.Notes, in.Score, in.ScoreSource, in.Subtype)
	if err != nil {
		return nil, err
	}
...
func (s *LibraryService) Import(ctx context.Context, in []CreateInput) (*ImportResult, error) {
	...
	for i, input := range in {
		item, err := NewItem(uuid.NewUUID(), input.Name, input.MediaType, input.ReleaseYear, input.Done, input.Notes, input.Score, input.ScoreSource, input.Subtype)
		if err != nil {
