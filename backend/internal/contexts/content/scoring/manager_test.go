...
func TestManager_ReloadEmpty_Deconfigures(t *testing.T) {
	...
	if err := m.Reload(context.Background(), config.ScoreProviderConfig{}); err != nil {
...
func TestManager_ReloadInvalid_KeepsPrevious(t *testing.T) {
	m := NewManager(nil)
	valid := config.ScoreProviderConfig{
		Game: "rawg",
		Keys: map[string]string{"rawg": "k"},
	}
	...
	invalid := config.ScoreProviderConfig{Movie: "netflix", Keys: map[string]string{"netflix": "k"}}
