...
func NewPostgresInsightRepository(pool *pgxpool.Pool) *PostgresInsightRepository {
	return &PostgresInsightRepository{pool: pool}
}

func (r *PostgresInsightRepository) SaveReport(ctx context.Context, report *InsightReport) error {
...
func (r *PostgresInsightRepository) GetReport(ctx context.Context, userID string, weekStart time.Time) (*InsightReport, error) {
...
func (r *PostgresInsightRepository) UpsertSession(ctx context.Context, session *PomodoroSessionRecord) error {
	_, err := r.pool.Exec(ctx, `
...
func (r *PostgresInsightRepository) ListSessions(ctx context.Context, userID string, from, to time.Time) ([]PomodoroSessionRecord, error) {
	...
	if sessions == nil {
		sessions = make([]PomodoroSessionRecord, 0)
	}
	return sessions, nil
}

func (r *PostgresInsightRepository) ListCompletions(ctx context.Context, from, to time.Time) ([]CompletionRecord, error) {
	rows, err := r.pool.Query(ctx, `
...
func (r *PostgresInsightRepository) CountRecipes(ctx context.Context, from, to time.Time) (int, error) {
	var count int
	err := r.pool.QueryRow(ctx, `
