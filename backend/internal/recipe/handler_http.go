...
func (h *Handler) CreateRecipe(w http.ResponseWriter, r *http.Request) {
	...
	shared.WriteJSON(w, http.StatusCreated, toResponse(recipe))
}
...
func (h *Handler) ListRecipes(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		shared.WriteError(w, http.StatusInternalServerError, "failed to list recipes")
		return
	}
	shared.WriteJSON(w, http.StatusOK, toResponses(recipes))
}
...
func (h *Handler) GetRecipe(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, ErrNotFound) {
		shared.WriteError(w, http.StatusNotFound, "recipe not found")
		return
	}
	if err != nil {
		log.Error().Err(err).Str("recipe_id", id).Msg("get recipe failed")
		shared.WriteError(w, http.StatusInternalServerError, "failed to get recipe")
		return
	}
	shared.WriteJSON(w, http.StatusOK, toResponse(recipe))
}
...
func (h *Handler) DeleteRecipe(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		shared.WriteError(w, http.StatusInternalServerError, "failed to delete recipe")
		return
	}
...
func (h *Handler) GenerateUploadURL(w http.ResponseWriter, r *http.Request) {
	...
	if req.ContentType == "" {
		shared.WriteError(w, http.StatusBadRequest, "content_type is required")
		return
	}
	...
	if err != nil {
		...
		shared.WriteError(w, http.StatusServiceUnavailable, "media uploads not available")
		return
	}
	shared.WriteJSON(w, http.StatusOK, upload)
}
