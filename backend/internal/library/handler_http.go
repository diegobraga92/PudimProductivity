...
func (h *Handler) Subtypes(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		shared.WriteError(w, http.StatusInternalServerError, "failed to list library subtypes")
		return
	}
	shared.WriteJSON(w, http.StatusOK, subtypes)
}
...
func (h *Handler) SearchScoresBatch(w http.ResponseWriter, r *http.Request) {
	...
	if len(req.Items) == 0 {
		shared.WriteError(w, http.StatusBadRequest, "no items to look up")
		return
	}
	if len(req.Items) > maxScoreBatchItems {
		shared.WriteError(w, http.StatusBadRequest, "too many items in a single batch (max 100)")
		return
	}
	...
	shared.WriteJSON(w, http.StatusOK, ScoreBatchResponse{Results: results})
}
