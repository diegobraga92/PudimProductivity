...
const itemColumns = `id, name, media_type, release_year, done, notes, created_at, updated_at`
...
func (r *PostgresRepository) Create(ctx context.Context, item *Item) error {
	_, err := r.pool.Exec(ctx, `
		INSERT INTO library_items (id, name, media_type, release_year, done, notes)
		VALUES ($1, $2, $3, $4, $5, $6)`,
		item.ID, item.Name, item.MediaType, item.ReleaseYear, item.Done, item.Notes,
	)
	if err != nil {
...
func (r *PostgresRepository) Update(ctx context.Context, item *Item) error {
	tag, err := r.pool.Exec(ctx, `
		...
		SET name = $2, media_type = $3, release_year = $4, done = $5, notes = $6, updated_at = NOW()
		WHERE id = $1`,
		item.ID, item.Name, item.MediaType, item.ReleaseYear, item.Done, item.Notes,
	)
	if err != nil {
