...
type Logger interface {
	...
	Log(ctx context.Context, action, resource, resourceID string, oldValues, newValues interface{})
}

// NoopLogger is a logger that discards all events (used when audit is not configured).
type NoopLogger struct{}

func (NoopLogger) Log(_ context.Context, _ string, _ string, _ string, _ interface{}, _ interface{}) {
	// no-op
}