...
func TestConnectPool_Select1(t *testing.T) {
	...
	// Use the real ConnectPool function from the production code.
...
// ExampleConnectPool demonstrates how to use ConnectPool with Testcontainers.
func ExampleConnectPool() {
	...
	pool, err := ConnectPool(ctx, connStr)
	if err != nil {
		fmt.Printf("ConnectPool failed: %v\n", err)
		return
	}
	defer pool.Close()
