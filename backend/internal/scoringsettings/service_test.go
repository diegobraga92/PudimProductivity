...
type fakeReloader struct {
	last shared.ScoreProviderConfig
	err  error
}

func (f *fakeReloader) Reload(_ context.Context, cfg shared.ScoreProviderConfig) error {
	if f.err != nil {
		return f.err
	}
	f.last = cfg
...
func TestService_ApplyConfig_InvalidEnvKeepsDegraded(t *testing.T) {
	repo := seededRepo()
	env := shared.ScoreProviderConfig{Movie: "netflix", Keys: map[string]string{"netflix": "k"}}
	reloader := &fakeReloader{}
	svc := NewService(repo, nil, reloader, nil, env)
...
func TestService_Config_MasksKeys(t *testing.T) {
	...
	svc := NewService(repo, nil, &fakeReloader{}, nil, shared.ScoreProviderConfig{})
